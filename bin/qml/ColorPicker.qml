...
Rectangle {
  ...
  MouseArea {
    ...
    ColorComponents {
      ...
      /*  onHueChanged: txtHue.text =qsTr(Math.floor(hue*360 ))
        onSaturationChanged: txtSaturation.text = qsTr(Math.round(saturation * 100))
        onValueChanged: txtValue.text = qsTr(Math.round(value * 100))

        onRedChanged: txtRed.text = qsTr(Math.round(red * 255))
        onGreenChanged: txtGreen.text =qsTr( Math.round(green * 255))
        onBlueChanged: txtBlue.text =qsTr( Math.round(blue * 255))

        onAlphaChanged: txtAlpha.text = qsTr(Math.round(alpha * 255))

        onColorChanged: {console.log('colorechanged');txtHex.text = qsTr(fullName);}*/
    }
            ...
            Row {
                ...
                Row {
                    ...
                    TextField {
                       ...
                       /* onTextChanged: {
                            if(Math.floor(currentColorComponents.hue * 360) !== parseInt(txtHue.text, 10))
