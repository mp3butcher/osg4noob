...
void  QtToOSGView::resizeEvent(QResizeEvent *event){
	...
	//
	/*
managedview->getCamera()->getGraphicsContext()->resized(p.x(),p.y(),s.width(),s.height());
		managedview->getCamera()->setViewport(p.x(),p.y(),s.width(),s.height());
			managedview->getCamera()->setProjectionMatrix( osg::Matrix::perspective( 30., (double)s.width()/(double)s.height(), 1., 500 ) );
	//
	std::cerr<<"resize QtOSGVIEW"<<p.x()<<" "<<p.y()<<" "<<s.width()<<" "<<s.height()<<endl;
	//	managedview->getEventQueue()->windowResize(p.x(),p.y(),p.x()+s.width(),p.y()+s.height());
