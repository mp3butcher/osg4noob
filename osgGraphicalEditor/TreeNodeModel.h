...
public:
    ...
    TreeNodeDataObject * _parentTreeNode;
    explicit TreeNodeModel(TreeNodeDataObject * parenttreeNode=0,QObject *parent = 0);
    ~TreeNodeModel() {}
    ...
    Q_INVOKABLE void  move(int from,int to,int atpos);
    ...
    */
    void getTreeNodeByName(QString hid,QList<TreeNodeDataObject*> &out)
    {
...
//		for tree in self._treeNodes:
            ...
            else
            {
                if ((*i)->_subnodes)
                {
                    //TreeNodeDataObject * ret=
                    (*i)->_subnodes->getTreeNodeByName( hid,out);
                    //  if (ret)return ret;
                }
            }
        }
...
QML_DECLARE_TYPE(TreeNodeModel)
